...
function atcb_generate_label_content(data, parent, type, icon, text, oneOption) {
  ...
  switch (type) {
    case "google":
    case "outlookcom":
      if (data.dates.length) {  // [CUSTOM] - Count the number of events in the calendar
        const counterEl = document.createElement('span');
        counterEl.classList.add('atcb-text');
        counterEl.innerHTML = data.dates.length ? "&nbsp;" + data.dates.length.toString() : "&nbsp; 0";
...
function atcb_create_atcbl(host, atList = true) {
  ...
  atcbL.id = 'add-button-to-calendar-reference';
